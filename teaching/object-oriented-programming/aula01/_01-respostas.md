...
> Arquivo de apoio consolidado, não publicado (prefixo `_`). Reúne o gabarito
> das questões de Verdadeiro/Falso e a solução das pausas ativas do `index.qmd`,
> seguindo a metodologia de criação de questões definida em `../../CLAUDE.md`.
> O `index.qmd` publicado permanece sem nenhuma dessas soluções — este arquivo
> existe só para conferência do professor/monitor.
