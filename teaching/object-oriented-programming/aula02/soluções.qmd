---
title: "Soluções — O Objeto como Máquina de Estados"
...
::: {.callout-tip}
## (Resposta) Teste 1 — Estado, Comportamento e Identidade (revisitados)
...
::: {.callout-tip}
## (Resposta) Teste 4 — A Máquina de Estados Finita (DFA)
...
- ✔ Verdadeiro — O diagrama de estados do `Produto` só define `vender(x<all)` (permanece em DISPONÍVEL) e `vender(all)` (vai para ESGOTADO); vender mais do que o estoque atual não tem nenhum destino previsto pela máquina. Como um DFA exige que toda combinação (estado, evento) leve a exatamente um estado, e essa combinação específica não tem destino legal no domínio de negócio, o código precisa recusar a chamada (`IllegalStateException`) em vez de deixar a máquina "vazar" para um estado matematicamente impossível (estoque negativo).
...
::: {.callout-tip}
## (Resposta) Teste 6 — Invariantes de Classe
...
::: {.callout-tip}
## (Resposta) Teste 7 — O Construtor como Base da Indução
...
::: {.callout-tip}
## (Resposta) Teste 8 — CQS: Comandos vs. Consultas
...
::: {.callout-tip}
## (Resposta) Teste 11 — Identidade Física vs. Lógica
