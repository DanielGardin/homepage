...
- □ Uma classe que exige "E" para ser descrita numa frase provavelmente viola o SRP.
- □ Injeção de Dependência via construtor garante que o objeto nunca nasça sem suas colaborações essenciais.
- □ Sem DI, testar a lógica de `Pedido` exigiria disparar um e-mail real ou cobrar um cartão de verdade.
- □ Usar um "Mock" no lugar de `Pagavel` real é uma violação do princípio de Injeção de Dependência.
:::
...
::: {.fragment}
**Voltando à pergunta:** sem DI, `Pedido` cria suas próprias dependências concretas internamente (`new CartaoDeCredito(...)`, `new ServicoEmail(...)`), então qualquer teste do método aciona o código real de pagamento e envio; com DI, o teste injeta mocks e isola só a lógica de orquestração.
:::
...
- □ Na Dependência, o objeto usado costuma ser recebido como parâmetro de método, não guardado como atributo.
- □ Na Agregação, destruir o "todo" implica destruir automaticamente a "parte".
- □ Na Composição, a "parte" não tem sentido de existir fora do "todo" — nasce e morre com ele.
- □ A escolha entre Agregação e Composição pode depender de uma regra de negócio, não só da sintaxe do código.
:::
...
::: {.fragment}
**Voltando à pergunta:** a declaração `private Cartao cartao` é idêntica sintaticamente nos dois casos — o que decide é a regra de domínio sobre o que deve acontecer ao cartão quando a conta é excluída; a mesma linha de código pode ser Agregação ou Composição dependendo dessa regra de negócio.
:::
...
## Do Naufrágio de Código à Delegação
...
::: {.fragment}
**A cura:** impedir que qualquer um precise do `Cartao` diretamente.
:::
...
- □ `joao.getCartao().processar(500.0)` respeita a Lei de Demeter, pois `Cartao` é um atributo de `Cliente`.
- □ `joao.pagar(500.0)` é um exemplo de "Tell": o chamador diz o que quer, sem saber como é feito.
- □ Depois da refatoração, o código que chama `pagar()` desconhece completamente a existência de `Cartao`.
- □ Associação (ter um objeto) e Delegação (usar esse objeto) juntas permitem trocar o `Cartao` sem afetar quem chama `pagar()`.
:::
...
::: {.fragment}
**Voltando à pergunta:** é o mesmo mecanismo — a delegação em cadeia dentro de `pagar()` respeita a Lei de Demeter porque `Cliente` só fala com seu próprio `Cartao` (um "amigo" direto), e é exatamente essa delegação que impede o chamador externo de navegar até `Cartao` e violar a lei.
:::
