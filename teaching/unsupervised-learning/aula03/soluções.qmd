...
- □ Se duas regiões de alta densidade são separadas por um vale de densidade exatamente zero em pelo menos um ponto do caminho entre elas, então, para qualquer $\lambda>0$, elas nunca aparecem como uma única componente conexa de $L_\lambda$.
...
::: {.callout-tip}
## (Resposta) Conjuntos de nível e a hierarquia indexada por $\lambda$
...
::: {.callout-tip}
## (Resposta) Core distance: densidade local como peso de aresta
...
::: {.callout-tip}
## (Resposta) Distância de alcançabilidade mútua
...
::: {.callout-tip}
## (Resposta) MST e a equivalência com ligação simples
...
::: {.callout-tip}
## (Resposta) O defeito do encadeamento (*chaining*)
...
::: {.callout-tip}
## (Resposta) Persistência por excesso de massa
...
- □ O cluster de $54$ pacientes, sendo $100\%$ malignos, é uma evidência de que o HDBSCAN funcionou como um classificador treinado no rótulo `diagnosis` — o rótulo foi usado apenas para conferir o resultado depois, não durante o clustering.
...
::: {.callout-tip}
## (Resposta) O resultado no Breast Cancer Wisconsin

- ✗ Falso — O item afirma duas coisas incompatíveis ao mesmo tempo. "Funcionar como um classificador treinado no rótulo" implicaria que o rótulo influenciou o ajuste — o que é falso: o HDBSCAN nunca viu `diagnosis` durante o clustering (só os atributos numéricos). O rótulo ter sido usado só para conferência posterior é justamente o que torna o resultado notável (estrutura real encontrada sem supervisão), não evidência de que funcionou "como classificador treinado" — a frase, como um todo, é falsa por misturar as duas ideias de forma incorreta.
...
::: {.callout-tip}
## (Resposta) Hiperparâmetros: `min_cluster_size` e `min_samples`
...
- □ Uma atribuição por protótipos fixos (distância euclidiana a centros) aplicada aos mesmos $30$ atributos padronizados está, em algum grau, imune à maldição da dimensionalidade, porque não usa nenhum estimador de densidade baseado em vizinhos como $\mathrm{core}_K$.
...
::: {.callout-tip}
## (Resposta) Maldição da dimensionalidade em clustering por densidade
