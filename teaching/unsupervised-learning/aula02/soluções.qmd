...
::: {.callout-tip}
## (Resposta) Concentração de volume na hiperesfera
...
::: {.callout-tip}
## (Resposta) As fórmulas do ESL (comprimento de aresta e distância mediana)
...
::: {.callout-tip}
## (Resposta) Concentração de medida e métricas de distância
...
- ✗ Falso — A tendência qualitativa (queda do contraste com $d$) persistiria, mas os valores numéricos específicos, não. Sem padronização, atributos de escala muito maior (ex.: uma variável na casa das centenas) dominariam completamente o cálculo de distância Euclidiana, distorcendo o contraste relativo medido — não seria "idêntico", como o item afirma.
:::
...
::: {.callout-tip}
## (Resposta) O resultado geral $p(\mathbf{x})=K/(NV)$
...
::: {.callout-tip}
## (Resposta) "Isto não é uma densidade de verdade"
...
::: {.callout-tip}
## (Resposta) Janela de Parzen e kernel gaussiano
...
::: {.callout-tip}
## (Resposta) $h$ como parâmetro de suavização
...
::: {.callout-tip}
## (Resposta) $k$-NN vs. KDE: suavização adaptativa vs. fixa
...
::: {.callout-tip}
## (Resposta) Custo computacional e armazenamento
...
::: {.callout-tip}
## (Resposta) Maldição da dimensionalidade em métodos não-paramétricos
...
::: {.callout-tip}
## (Resposta) $d_K(\mathbf{x})$ como métrica reaproveitável
