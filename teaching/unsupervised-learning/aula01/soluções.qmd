...
::: {.callout-tip}
## (Resposta) Profiling sem rótulos e distribuição empírica vs. teórica
...
- □ Ao descrever a distribuição conjunta de altura e peso de uma população adulta com uma Gaussiana bivariada, os autovetores de $\hat\Sigma$ ainda dariam a orientação dos eixos de densidade constante e os autovalores ainda dariam o comprimento desses eixos — exatamente como aconteceria ao modelar duas outras variáveis correlacionadas, como as leituras de temperatura e vibração de um equipamento, com uma Gaussiana bivariada.
...
::: {.callout-tip}
## (Resposta) A Gaussiana multivariada: parâmetros e geometria
...
::: {.callout-tip}
## (Resposta) Por que a Gaussiana (entropia máxima e Teorema Central do Limite)
...
- □ Um analista que ajusta $\hat{\boldsymbol\mu}$ e $\hat\Sigma$ a partir de uma amostra de retornos financeiros diários obteria o mesmo tipo de viés sistemático — subestimação da covariância verdadeira por um fator $(N-1)/N$ — pela mesma razão matemática, independentemente do tipo de dado.
...
::: {.callout-tip}
## (Resposta) Ajuste por máxima verossimilhança
...
::: {.callout-tip}
## (Resposta) A distância de Mahalanobis
...
::: {.callout-tip}
## (Resposta) Os pontos B e C: geometria da crista
...
::: {.callout-tip}
## (Resposta) Do limiar fixo ao $p$-valor
...
::: {.callout-tip}
## (Resposta) A armadilha de interpretação do $p$-valor
...
- ✗ Falso — "Surpreendente sob o modelo ajustado" é uma afirmação sobre o modelo; "não pertence à população verdadeira" é uma afirmação sobre a realidade — as duas só coincidem se o modelo estiver certo, uma suposição adicional que o item tenta remover ("sem mais suposições").
:::
...
::: {.callout-tip}
## (Resposta) Conjunta vs. por dimensão
...
::: {.callout-tip}
## (Resposta) Outliers no ajuste e ponte para a Aula 2
