...
::: {.callout-tip}
## (Resposta) Existência e formato da SVD
...
::: {.callout-tip}
## (Resposta) Construção via $A^TA$ e $AA^T$
...
::: {.callout-note icon=false}
## Valores singulares e norma espectral
...
::: {.callout-tip}
## (Resposta) Valores singulares e norma espectral

- ✔ Verdadeiro — Se $\sigma_1=\dots=\sigma_n=c$, então $\Sigma=cI$ e $A=U(cI)V^T=c(UV^T)=cQ$, com $Q=UV^T$ ortogonal. Como $Q$ ortogonal preserva norma para **qualquer** vetor ($\|Q\mathbf{x}\|_2=\|\mathbf{x}\|_2$), segue $\|A\mathbf{x}\|_2=c\|Q\mathbf{x}\|_2=c\|\mathbf{x}\|_2$ para todo $\mathbf{x}$ — não apenas no ponto de máximo.
...
::: {.callout-note icon=false}
## Aproximação de baixo posto e SVD truncada
...
::: {.callout-tip}
## (Resposta) Aproximação de baixo posto e SVD truncada
...
- □ Se $A$ já for ortogonal ($A^TA=I$), sua Decomposição Polar tem $S=I$ e $Q=A$.
- □ Se todos os valores singulares de $A$ forem iguais entre si ($\sigma_1=\dots=\sigma_n=c$), o fator $S$ da Decomposição Polar é exatamente $cI$ (um múltiplo escalar da identidade).
...
::: {.callout-tip}
## (Resposta) SVD vs. decomposição espectral
...
::: {.callout-note icon=false}
## A SVD aplicada a $X$: posto e número de condição
...
::: {.callout-tip}
## (Resposta) A SVD aplicada a $X$: posto e número de condição
...
::: {.callout-note icon=false}
## Filtragem colaborativa e interpretação de perfis latentes
...
::: {.callout-tip}
## (Resposta) Filtragem colaborativa e interpretação de perfis latentes
