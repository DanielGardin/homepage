...
::: {.callout-tip}
## (Resposta) Árvores como estimação não-paramétrica
...
::: {.callout-tip}
## (Resposta) O estimador ótimo numa folha de regressão
...
::: {.callout-tip}
## (Resposta) Crescimento guloso e sua miopia
...
::: {.callout-tip}
## (Resposta) Entropia e informação mútua
...
::: {.callout-tip}
## (Resposta) Ganho de informação e a conexão MLE categórico–entropia
...
- ✔ Verdadeiro — Nada na derivação do MLE categórico é específico de duas classes — o argumento de maximização da log-verossimilhança de uma distribuição categórica vale para $K$ classes quaisquer, e o máximo continua sendo a proporção empírica de cada uma. A fórmula $H(p)=-\sum_k p_k\ln p_k$ já é escrita para $K$ genérico; um exemplo binário é só uma escolha de ilustração, não uma restrição do resultado.
...
::: {.callout-tip}
## (Resposta) Entropia e Gini: forma e interpretação
...
::: {.callout-tip}
## (Resposta) Por que não usar a taxa de erro bruta para crescer a árvore
...
::: {.callout-tip}
## (Resposta) Custo vs. complexidade e a poda
...
::: {.callout-tip}
## (Resposta) Treino vs. validação: por que separar os dados

- ✔ Verdadeiro — Com $0\%$ separado, o conjunto de "validação" coincide com o próprio conjunto de treino — medir desempenho nele é, por construção, medir desempenho nos mesmos dados usados para ajustar o modelo, o caso mais otimista possível de estimativa de desempenho.
...
::: {.callout-tip}
## (Resposta) Splits alinhados aos eixos
...
::: {.callout-tip}
## (Resposta) Instabilidade estrutural
...
::: {.callout-tip}
## (Resposta) Partição rígida e descontinuidades
